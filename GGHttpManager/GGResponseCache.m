...
static NSString * const GGRequestManagerCacheDirectory = @"GGRequestCacheDirectory";

@implementation GGResponseCache
...
+ (dispatch_queue_t)cacheQueue {
    ...
    dispatch_once(&onceToken, ^{
        cacheQueue = dispatch_queue_create("com.GGResponseCache.cahceQueue", DISPATCH_QUEUE_CONCURRENT);
...
- (NSString *)creatCacheDirectory
{
    ...
    NSString *cachePath = [cachePathDic stringByAppendingPathComponent:GGRequestManagerCacheDirectory];
    BOOL isDirectory;
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    dispatch_async([self queue], ^{
        ...
        if (!written) {
            NSLog(@"*** cache Object to file failed!");
        }
    });
}
...
- (void)removeObjectForKey:(NSString *)key {
    ...
    if ([self.fileManager fileExistsAtPath:filePath]) {
        ...
        if (!removed) {
            NSLog(@"ERROR--> remove item failed with error: %@: %@",filePath, error);;
        }
    }
}
...
- (void)removeAllObjects {
    ...
    if (!removed) {
        NSLog(@"ERROR--> remove cache directory failed with %@: %@",self.cachePath, error);;
    }
}


#pragma mark - private method
- (NSDate *)cacheDateFilePath:(NSString *)filePath {
    NSError *attributeRetrievalError = nil;
    NSDictionary *attributes = [self.fileManager attributesOfItemAtPath:filePath error:&attributeRetrievalError];
    if (!attributes) {
        NSLog(@"ERROR --> Get attributes for file at %@: %@",filePath,attributeRetrievalError);
        return nil;
    }
...
- (void)trimToDate:(NSDate *)date {
    ...
    __autoreleasing NSError *error = nil;
    NSArray *files = [self.fileManager contentsOfDirectoryAtURL:[NSURL URLWithString:self.cachePath] includingPropertiesForKeys:@[NSURLContentModificationDateKey] options:NSDirectoryEnumerationSkipsHiddenFiles error:&error];
    if (error) {
        NSLog(@"ERROR--> get file error: %@",error);
    }
